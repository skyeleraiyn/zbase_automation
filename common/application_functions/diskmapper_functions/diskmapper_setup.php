...
class diskmapper_setup{
	...
	public function reset_diskmapper_storage_servers($SS_pool = NULL, $clear_vb_map = True){
		self::kill_zstore_cmd();
		if($SS_pool != NULL) {
              		storage_server_setup::reset_dm_storage_servers($SS_pool, $clear_vb_map);
		}
		else {
			storage_server_setup::reset_dm_storage_servers(NULL, $clear_vb_map);
		}
