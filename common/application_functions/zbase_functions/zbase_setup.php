...
class zbase_setup{
	...
	public function clear_zbase_log_file($remote_machine){
		file_function::clear_log_files($remote_machine, ZBASE_LOG_FILE);
	}
	...
	public function stop_zbase_server_service($remote_machine_name){
		service_function::control_service($remote_machine_name, ZBASE_SERVER_SERVICE, "stop");
	}
	...
	public function kill_zbase_server($remote_machine_name) {
		return process_functions::kill_process($remote_machine_name, MEMCACHED_PROCESS);
	}

	public function clear_cluster_zbase_database($reset_spare = False){
		global $test_machine_list;
		global $spare_machine_list;
		if($reset_spare) {
			foreach($spare_machine_list as $spare) {
				self::clear_zbase_database($spare);
			}
		}
		$pid_arr = array();
		foreach ($test_machine_list as $test_machine) {
			$pid = pcntl_fork();
			if($pid == 0) {
				self::clear_zbase_database($test_machine);
				exit();
			}
			else {
				$pid_arr[]=$pid;
	...
	public function clear_zbase_database($remote_machine_name) {
		// To ensure root folder doesn't get deleted if the constant is not defined
		if(!(defined('ZBASE_DATABASE_PATH')) or ZBASE_DATABASE_PATH == ""){
			log_function::result_log("Constant ZBASE_DATABASE_PATH is not defined"); 
			exit;
		}
		remote_function::remote_execution($remote_machine_name, "sudo mount -a ");
		foreach(unserialize(ZBASE_DATABASE_PATH) as $zbase_dbpath){
			if($zbase_dbpath == ""){
				log_function::result_log("zbase_dbpath is not defined"); 
				exit;
			}
		
			for($iattempt = 0 ; $iattempt < 60 ; $iattempt++) {
				if (stristr(remote_function::remote_execution($remote_machine_name, "ls ".$zbase_dbpath), "No such file or directory")) {
					remote_function::remote_execution($remote_machine_name, "sudo mkdir ".$zbase_dbpath);
					remote_function::remote_execution($remote_machine_name, "sudo chown -R nobody ".$zbase_dbpath);
					break;
				} else {
					if(stristr(remote_function::remote_execution($remote_machine_name, "ls ".$zbase_dbpath), "sqlite")) {
						remote_function::remote_execution($remote_machine_name, "sudo rm -rf ".$zbase_dbpath."/*");
						sleep(5);
					} else {
	...
	public function reset_servers_and_backupfiles($master_server, $slave_server){
		self::reset_zbase_vbucketmigrator($master_server, $slave_server);
		zbase_backup_setup::stop_backup_daemon($slave_server);
	...
	public function reset_zbase_vbucketmigrator($master_server, $slave_server) {
		self::reset_zbase_servers(array($master_server, $slave_server));
	...
	public function reset_zbase_servers($remote_machine_array_list, $clear_db = True){
		$pid_count = 0;
		foreach ($remote_machine_array_list as $remote_machine){
			$pid = pcntl_fork();
			if ($pid == 0){	
				if(!(self::kill_zbase_server($remote_machine))){
					log_function::result_log("Failed to terminate zbase on $remote_machine");
					exit(4);
				}
				sleep(2);
				if($clear_db){
					self::clear_zbase_log_file($remote_machine);
					if(!(self::clear_zbase_database($remote_machine))){
				...
				if(!(self::memcached_service($remote_machine, "start"))){
					log_function::result_log("Failed to start zbase on $remote_machine");
					exit(4);
				}
				sleep(1);
	...
	public function restart_zbase_cluster($restart_spare = False){
		global $test_machine_list;
		global $spare_machine_list;
		if($restart_spare) {
			foreach($spare_machine_list as $spare) {
				self::restart_zbase_servers($spare);
			}
		}
		...
		foreach ($test_machine_list as $test_machine) {
			$pid = pcntl_fork();
			if($pid == 0) {
	                        self::restart_zbase_servers($test_machine);
				exit();
			}
			else {
				$pid_arr[] = $pid;
	...
	public function restart_zbase_servers($remote_machine_name){
		...
		log_function::debug_log("Zbase failed to restart $remote_machine_name");		
		return False;
	}
	...
	public function add_log_filter_rsyslog($remote_machine_name){
		...
		$zbase_log = file_function::query_log_files($rsysconfig_file, "zbase", $remote_machine_name);
		if(!stristr($zbase_log, "zbase")){
			general_function::execute_command("sudo sh -c 'echo :msg, contains, \"zbase\" >> '".$rsysconfig_file, $remote_machine_name);
			general_function::execute_command("sudo sh -c 'echo \*\.\* /var/log/zbase.log >> '".$rsysconfig_file."' ; echo \"\" >> '".$rsysconfig_file, $remote_machine_name);
		}	
	
		$zbasebackup_log = file_function::query_log_files($rsysconfig_file, "ZbaseBackup", $remote_machine_name);
		if(!stristr($zbasebackup_log, "ZbaseBackup")){
			general_function::execute_command("sudo sh -c 'echo :msg, contains, \"ZbaseBackup\" >> '".$rsysconfig_file, $remote_machine_name);
			general_function::execute_command("sudo sh -c 'echo \*\.\* /var/log/zbasebackup.log >> '".$rsysconfig_file."' ; echo \"\" >> '".$rsysconfig_file, $remote_machine_name);
		}	
