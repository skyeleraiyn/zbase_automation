...
class storage_server_setup{
	...
	public function reset_dm_storage_servers($storage_server_list = NULL, $clear_vb_map = True){
		...
		foreach($storage_server_list as $storage_server){
				$pid = pcntl_fork();
				if($pid == 0){
						self::clear_storage_server_data_folders($storage_server, $clear_vb_map);
	...
	public function clear_storage_server_data_folders($storage_server, $clear_vb_map = True) {
        if($clear_vb_map) {
            $command_to_be_executed = "sudo rm -rf /data_*/*/*;sudo rm -rf /var/www/html/".GAME_ID;
        }
        else {
    		$command_to_be_executed = "sudo rm -rf /data_*/*/*/*/*;";
        }
		return remote_function::remote_execution($storage_server, $command_to_be_executed);
	}
	...
	public function clear_bad_disk_entry($storage_server){
		$command_to_be_executed = "cat /dev/null | sudo tee /var/tmp/disk_mapper/bad_disk ; sudo chown storageserver /var/tmp/disk_mapper/bad_disk;sudo mount -a";
		return remote_function::remote_execution($storage_server, $command_to_be_executed);
	}
