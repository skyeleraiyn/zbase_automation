...
class synthetic_backup_generator{
	...
	public function prepare_merge_backup($hostname, $type)	{
		...
		$command_to_be_executed = "sudo mkdir -p /$disk/primary/$host[0]/".MEMBASE_CLOUD."; sudo chown -R storageserver.storageserver /$disk/primary/;";
		...
		if(trim(remote_function::remote_execution($storage_server, $command_to_be_executed)) == 0)	{ 
			$command_to_be_executed = "php /tmp/generate_merge_data.php $type 5 1000000 10 1000 $disk $host[0] $cloud";
			$status = remote_function::remote_execution($storage_server, $command_to_be_executed);
			return $status;
		}
		$command_to_be_executed = "sudo cp -R $backup_path/* /$disk/primary/$host[0]/$cloud/; sudo chown -R storageserver.storageserver /$disk/primary/$host[0]/$cloud/";
		remote_function::remote_execution($storage_server, $command_to_be_executed);
		return 1;
