...
class generate_ssh_key{
	...
	public function copy_public_key_to_remote_machines($remote_machine_list){
		...
		$user_id_in_public_key = explode(" ", $public_key_contents);
		$user_id_in_public_key = $user_id_in_public_key[2];
		var_dump($user_id_in_public_key);
		...
		foreach($remote_machine_list as $remote_machine){	
			...
			var_dump($auth_key_output);
			var_dump(stristr($auth_key_output, trim($user_id_in_public_key)));

			if(!stristr($auth_key_output, trim($user_id_in_public_key))){
