...
class general_function{
	...
	public function get_dual_ip($ip)	{
		$return_array = array();
		$command_to_be_executed = "/sbin/ifconfig eth0 | grep 'inet addr' | tr -s ' ' | awk -F ':' '{ print $2}' | awk '{print $1}'" ;
		$returned_ip_eth0 = remote_function::remote_execution($ip, $command_to_be_executed);
		$command_to_be_executed = "/sbin/ifconfig eth1 | grep 'inet addr' | tr -s ' ' | awk -F ':' '{ print $2}' | awk '{print $1}'";
		$returned_ip_eth1 = remote_function::remote_execution($ip, $command_to_be_executed);
		$return_array[0] = $returned_ip_eth0;
		$return_array[1] = $returned_ip_eth1;
		return $return_array;
	}
	...
	public function initial_setup($remote_machine_list){	
		...
		if(IBR_STYLE == 2.0 && defined('DISK_MAPPER_SERVER_PASSIVE') && DISK_MAPPER_SERVER_PASSIVE <> ""){
