...
while :; do
    ...
    if [[ -n "$USER" ]];then
        su -s /bin/bash - $USER -c "/opt/membase/bin/vbucketmigrator -N $TAPNAME -d $SLAVE  -r -h 127.0.0.1:11211 -b 0 -A -v $OPTIONS"
    else
        /opt/membase/bin/vbucketmigrator -N $TAPNAME -d $SLAVE -r -h 127.0.0.1:11211 -b 0 -A -v $OPTIONS
    fi
