...
class Performance_function{
	...
	public function run_performance_test($data_sample){
		...
		foreach ($data_sample as $data_size => $total_no_of_keys) {
			...
			while(1){			// Resets if ep_item_commit_failed starts increasing while running set_get.php
				zbase_setup::clear_zbase_log_file(MASTER_SERVER);
				vbucketmigrator_function::clear_vbucketmigrator_log_file(MASTER_SERVER);
				zbase_setup::clear_zbase_log_file(SLAVE_SERVER_1);
				
				zbase_setup::reset_zbase_servers(array(MASTER_SERVER, SLAVE_SERVER_1));
			...
			self::restart_master_zbase_to_capture_warmup_time();
	...
	public function restart_master_zbase_to_capture_warmup_time(){
		
		zbase_setup::kill_zbase_server(MASTER_SERVER);
		zbase_setup::memcached_service(MASTER_SERVER, "start");
		for($iTime = 0 ; $iTime < 1080 ; $iTime++){
			...
			if (stristr($output, "ep_warmup_time")){
				...
				log_function::result_log("Zbase memory of ".MASTER_SERVER." after warmup: ".zbase_function::get_zbase_memory(MASTER_SERVER, "GB")."GB");
				return 1;
			}	
			else
				sleep(10);
		}	
		log_function::debug_log("Unable to connect to zbase server on ".MASTER_SERVER." after restart");		
	}
	...
	public function reset_slave_and_reattach_it_master_server($total_no_of_keys) {
		...
		while(1){
			zbase_setup::reset_zbase_servers(array(SLAVE_SERVER_1));
			sleep(10);
	...
	public function capture_stats_log_files(){
		...
		zbase_function::copy_zbase_log_file(MASTER_SERVER, $data_folder."/".MASTER_SERVER);
		zbase_function::copy_zbase_log_file(SLAVE_SERVER_1, $data_folder."/".SLAVE_SERVER_1);
	...
	public function capture_stats_log_files_fresh_replication(){
		...
		zbase_function::copy_zbase_log_file(MASTER_SERVER, $data_folder."/".MASTER_SERVER."_freshreplication");
		zbase_function::copy_zbase_log_file(SLAVE_SERVER_1, $data_folder."/".SLAVE_SERVER_1."_freshreplication");
	...
	public function capture_stats_log_files_warmup(){
		...
		zbase_function::copy_zbase_log_file(MASTER_SERVER, $data_folder."/".MASTER_SERVER."_warmup");
	...
	public function set_get($total_no_of_keys, $data_size){
		...
		foreach($ip_address_list as $ip_address){
			$mc_master->addserver($ip_address, ZBASE_PORT_NO);
		}	
		$mc_slave_1 = new Memcache();
		$mc_slave_1->addserver(SLAVE_SERVER_1, ZBASE_PORT_NO);
		...
		/* 	Check if ep_item_commit_failed increases due to storage errors at the start of zbase server
		...
		log_function::result_log("Zbase memory of master_server ".MASTER_SERVER.": ".zbase_function::get_zbase_memory(MASTER_SERVER, "GB")."GB");
		log_function::result_log("Zbase DB size on disk for master_server ".MASTER_SERVER.": ".zbase_function::get_zbase_db_size(MASTER_SERVER));
		log_function::result_log("Zbase memory of slave_server ".SLAVE_SERVER_1.": ".zbase_function::get_zbase_memory(SLAVE_SERVER_1, "GB")."GB");
		log_function::result_log("Zbase DB size on disk for slave_server ".SLAVE_SERVER_1.": ".zbase_function::get_zbase_db_size(SLAVE_SERVER_1));
		log_function::result_log("get misses as reported by Zbase: ".$stats["get_misses"]);
