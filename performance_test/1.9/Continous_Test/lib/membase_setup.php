...
class membase_setup{
	...
	public function clear_restart_membase($remote_machine_name) {
		$command_to_be_executed = "sudo /etc/init.d/memcached stop; sudo killall python;sudo /etc/init.d/vba stop; for i in {1..3}; do sudo rm /data_\$i/membase/* -f; done;sleep 10;sudo cat /dev/null|sudo tee /var/log/membase.log;sudo /etc/init.d/memcached start; sudo /etc/init.d/memcached start";
