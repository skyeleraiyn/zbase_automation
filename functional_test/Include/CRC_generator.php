...
class CRC_generator{
	...
	public function getCksum($flag, $testvalue) {
		$crc = 0xffffffff;		
		$value_length = strlen($testvalue);
		...
		for ($i = 0; $i<4; $i++) {
			$crc = ($crc >> 8) ^ $crc32tab[($crc ^ (($flag & (0xff << (8*$i)))>>(8*$i))) & 0xff];
		}

		for ($i = 0; $i<$value_length; $i++) {
			$crc = ($crc >> 8) ^ $crc32tab[($crc ^ ord($testvalue[$i])) & 0xff];
		}
		$out = sprintf("%x", ~$crc & 0xffffffff);
		return $out;
	}
