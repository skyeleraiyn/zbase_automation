...
abstract class Restore_TestCase extends ZStore_TestCase {
	...
	public function test_Restore_Invalid_HostName(){
		...
		$command_to_be_executed = "sudo python26 ".MEMBASE_RESTORE_SCRIPT." -h rand_serv";
	...
	public function test_Restore_Invalid_Cloud_and_GameID(){ 
		...
		//EXPECTED RESULT : Restore fails
		membase_function::reset_servers_and_backupfiles(TEST_HOST_1, TEST_HOST_2);
		mb_backup_commands::edit_defaultini_file('cloud' , 'random_cld');
		mb_backup_commands::edit_defaultini_file('game_id' , 'random_game');
		$status = mb_restore_commands::restore_server(TEST_HOST_2);
		$this->assertTrue(strpos($status,"Could not find any incremental backups")>0 , "Missing incremental backups not reported");
		$this->assertTrue(strpos($status,"Could not find master backup. Maximum tries exceeded")>0, "Missing master backups not reported");
		$this->assertTrue(strpos($status,"No backup files found.")>0, "Missing backups not reported");
	...
	public function test_Verify_Lock_File(){ 
		...
		if($pid == -1){
			...
			$lock_file = mb_backup_commands::list_incremental_backups("lock*");
	...
	public function test_Restore_Membase_stopped(){
		// AIM : Run restore without membase running on the box
		//EXPECTED RESULT : the restore process starts membase before proceeding with the restore
		membase_function::reset_servers_and_backupfiles(TEST_HOST_1, TEST_HOST_2);
		flushctl_commands::set_flushctl_parameters(TEST_HOST_1, "chk_max_items", 100);
		...
		$this->assertTrue(strpos($memcached_status, "is running")>0 , "Membase not started");
		$this->assertTrue(strpos($status,"Starting membase")>0,"Membase not started");
		//Verifying count of keys
		$count = stats_functions::get_all_stats(TEST_HOST_2, "curr_items");
	...
	public function test_Restore_Only_Incremental_Backups(){
		...
		mb_backup_commands::delete_master_backups();
		membase_function::reset_membase_servers(array(TEST_HOST_1, TEST_HOST_2));
		$status = mb_restore_commands::restore_server(TEST_HOST_2);
		$this->assertTrue(strpos($status,"Restore completed successfully")>0,"Restore not completed");
	}
	...
	public function test_Restore_Only_Master_Backups(){
		...
		mb_backup_commands::delete_incremental_backups();
		membase_function::reset_membase_servers(array(TEST_HOST_1, TEST_HOST_2));
		$status = mb_restore_commands::restore_server(TEST_HOST_2);
		$this->assertTrue(strpos($status,"Restore completed successfully")>0,"Restore not completed");
		//Verifying count of keys
	...
	public function test_Restore_Missing_Split_Files_in_MasterDir(){
		...
		$status = mb_backup_commands::run_master_merge();
		$this->assertTrue(strpos($status, "Success: Master merge completed")>0, "Master Merge not done");
		$split_files[0] = mb_backup_commands::list_master_backups(".split", date("Y-m-d"));
		$split_files[1] = mb_backup_commands::list_master_backups(".split", date("Y-m-d",mktime(0,0,0,date("m"),date("d")-7,date("Y"))));
	...
	public function test_Restore_One_Buffer_Defined(){
		// AIM : Run restore process with only one buffer defined in the /etc/membase-backup/default.ini file
		// EXPECTED RESULT : Restore completes successfully
		membase_function::reset_servers_and_backupfiles(TEST_HOST_1, TEST_HOST_2);
		flushctl_commands::set_flushctl_parameters(TEST_HOST_1, "chk_max_items", 100);
		$this->assertTrue(Data_generation::add_keys(100, 100, 1, 10),"Failed adding keys");
		...
		$this->assertTrue(mb_backup_commands::verify_membase_backup_success(), "Failed to upload the backup files to Storage Server");
		membase_function::reset_membase_servers(array(TEST_HOST_1, TEST_HOST_2));
		mb_backup_commands::edit_defaultini_file('buffer_list' , '\/db_backup\/0');
		membase_function::reset_membase_servers(array(TEST_HOST_1, TEST_HOST_2));
		$status = mb_restore_commands::restore_server(TEST_HOST_2);
	...
	public function test_Restore_Corrupt_backups(){
		...
		$status = mb_backup_commands::run_daily_merge();
		$this->assertTrue(strpos($status, "Merge complete")>0, "Daily merge not completed");
		$backup = mb_backup_commands::list_master_backups(".mbb", date("Y-m-d"));
		sqlite_functions::corrupt_sqlite_file(STORAGE_SERVER, $backup[0]);
		membase_function::reset_membase_servers(array(TEST_HOST_1, TEST_HOST_2));
		$status = mb_restore_commands::restore_server(TEST_HOST_2);
	...
	public function test_Two_Instances_for_Same_Host(){
		...
		membase_function::reset_servers_and_backupfiles(TEST_HOST_1, TEST_HOST_2);
		membase_function::clear_membase_backup_log_file(TEST_HOST_2);
		flushctl_commands::set_flushctl_parameters(TEST_HOST_1, "chk_max_items", 100);
		...
		$command_to_be_executed = "cat ".MEMBASE_BACKUP_LOG_FILE;
	...
	public function test_Restore_Two_Slaves_Both_Reading_Same_Dir(){ //check
		...
		// Restore both
		membase_function::reset_membase_servers(array(TEST_HOST_1, TEST_HOST_2));
		storage_server::configure_storage_server(TEST_HOST_1 , STORAGE_SERVER);
		$pid = pcntl_fork();
		if($pid){		//parent
			// run the restore script on master
			log_function::debug_log("Parent running restore script on master");
			$command_to_be_executed = "sudo python26 ".MEMBASE_RESTORE_SCRIPT." -h ".TEST_HOST_2;
		...
		sleep(3);
		$lock_file = mb_backup_commands::list_incremental_backups("lock*");// lock file is not created
	...
	public function test_Backfill_from_Restore(){
		...
		mb_backup_commands::clear_storage_server();
		mb_backup_commands::stop_backup_daemon(TEST_HOST_2);
		membase_function::restart_memcached_service(TEST_HOST_2); 
		tap_commands::register_backup_tap_name(TEST_HOST_2); 
		mb_backup_commands::start_backup_daemon_full(TEST_HOST_2);
		sleep(10);
		// Restore
