...
abstract class Basic_TestCase extends ZStore_TestCase {
	...
	public function test_Kill_Vbucketmigrator()
	{
		...
		cluster_setup::setup_zbase_cluster($vbuckets = NO_OF_VBUCKETS,$restart_spares = True);
		sleep(100);
	...
	public function test_Behavior_Diskdown_Diskup()
	{
		...
		$this->assertEquals($flag,true,"Vbuckets are not marked dead and zbase didnt recognise disk failure");	
	...
	//Trestcase to verify zbase volatile failure
	public function test_Zbase_Failure_Volatile()
	{
		global $moxi_machine;
		global $test_machine_list;
		cluster_setup::setup_zbase_cluster($vbuckets = NO_OF_VBUCKETS,$restart_spares = True);
	...
	//Testcase to verify zbase connectivity failure is detected or not	
	public function test_Break_Zbase_Connectivity()
	{
		global $test_machine_list;
		cluster_setup::setup_zbase_cluster($vbuckets = NO_OF_VBUCKETS,$restart_spares = True);
		sleep(100);
	...
	public function  test_Bring_Down_VBA()
	{
		...
		//Verify the replica vbuckets for  the active in the down zbase became active or not 
		foreach($vbucket_active_info as $vb_id=>$value)
			{
				$falg = true;
	...
	//Bring down zbase and verify the vbuckets get redistributed properly	
	public function test_Bring_Down_Zbase()
	{
		global $test_machine_list;
		cluster_setup::setup_zbase_cluster($vbuckets = NO_OF_VBUCKETS,$restart_spares = True);
		sleep(100);
