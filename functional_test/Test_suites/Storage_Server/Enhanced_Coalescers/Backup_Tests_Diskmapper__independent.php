...
abstract class Backup_Tests_Diskmapper  extends ZStore_TestCase {                             
	...
	public function test_delete_local_backup_folder() {
		membase_setup::reset_servers_and_backupfiles(TEST_HOST_1,TEST_HOST_2);
		remote_function::remote_execution(TEST_HOST_2, "sudo rm -rf ".MEMBASE_DB_LOCAL_BACKUP_FOLDER);
		flushctl_commands::set_flushctl_parameters(TEST_HOST_1, "chk_max_items", 100);
		$this->assertTrue(Data_generation::add_keys(100, 100),"Failed adding keys");
		...
		$this->assertTrue(backup_tools_functions::verify_membase_backup_upload(), "Failed to upload the first backup files to Storage Server");
		...
		membase_backup_setup::restart_backup_daemon(TEST_HOST_2);
		$this->assertTrue(backup_tools_functions::verify_membase_backup_upload(), "Failed to upload the second backup files to Storage Server");
		$failure = backup_tools_functions::upload_stat_from_membasebackup_log("FAILED");	
		$error = backup_tools_functions::upload_stat_from_membasebackup_log("ERROR");	
	...
	public function test_upload_retry() {
		...
		sleep(60);
		$log_retry = backup_tools_functions::upload_stat_from_membasebackup_log("Retrying", 100);
		$log_failed = backup_tools_functions::upload_stat_from_membasebackup_log("FAILED", 10);
		$array_retry = explode("\n",$log_retry);
	...
	public function test_upload_to_new_primary() {
		...
		membase_backup_setup::restart_backup_daemon(TEST_HOST_2);
		sleep(60);
		$log_change = backup_tools_functions::upload_stat_from_membasebackup_log("Diskmapper hostconfig has changed");
		$log_upload = backup_tools_functions::upload_stat_from_membasebackup_log("Uploading missing backups");
	...
	public function test_upload_when_primary_rehydrating() {
		...
		membase_backup_setup::restart_backup_daemon(TEST_HOST_2);
		sleep(60);
		$log_change = backup_tools_functions::upload_stat_from_membasebackup_log("Diskmapper hostconfig has changed",10);
		$this->assertTrue(strpos($log_change,"Executing localbackup based resolution") > 0, "config change not detected");	
		$this->assertTrue(Data_generation::add_keys(100, 100, 401),"Failed adding keys");
		$this->assertTrue(torrent_functions::wait_for_torrent_copy(enhanced_backup_functions::trim_slave_host_name(),180) , "Failed to copy file to new disk");
		membase_backup_setup::restart_backup_daemon(TEST_HOST_2);
		sleep(60);
		$log_change_2 = backup_tools_functions::upload_stat_from_membasebackup_log("Diskmapper hostconfig has changed",10);
		$this->assertTrue(strpos($log_change,"Executing localbackup based resolution") > 0, "config change not detected");
		$change_arr = explode("\n", $log_change_2);
	...
	public function test_delete_from_new_primary() {
		...
		$storage_map = diskmapper_functions::get_primary_partition_mapping(enhanced_backup_functions::trim_slave_host_name());
		$path = "/".$storage_map['disk']."/primary/".enhanced_backup_functions::trim_slave_host_name()."/".MEMBASE_CLOUD."/incremental/";
	...
	public function test_primary_down_while_uploading() {
		...
		$missing = backup_tools_functions::upload_stat_from_membasebackup_log("Uploading missing backups");
		$this->assertTrue(strpos($missing,"Uploading missing backups") > 0, "previous backups not re uploaded");
		$log = backup_tools_functions::upload_stat_from_membasebackup_log($latest_backup, 2);
