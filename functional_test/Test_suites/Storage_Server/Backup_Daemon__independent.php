...
abstract class IBR_BackupDaemon_TestCase extends ZStore_TestCase {	
	...
	public function test_Membasebackup_Log() {
		...
		membase_backup_setup::start_backup_daemon(TEST_HOST_2);
		$this->assertTrue(backup_tools_functions::verify_membase_backup_upload(), "Failed to upload the backup files to Storage Server");
		$array = backup_tools_functions::upload_stat_from_membasebackup_log("SUCCESS");
	...
	public function test_Two_Backups_One_Chkpoint_Each() {	
		...
		$this->assertTrue(membase_backup_setup::start_backup_daemon(TEST_HOST_2), "Failed uploading the backups");
		$this->assertTrue(backup_tools_functions::verify_membase_backup_upload(), "Failed to upload the backup files to Storage Server");
		$array = storage_server_functions::list_master_backups();
		$count_backup = membase_function::sqlite_cpoint_count(STORAGE_SERVER_1, $array[0]);
		$count_backup += membase_function::sqlite_cpoint_count(STORAGE_SERVER_1, $array[1]);
		...
		sleep(10);
		$chk_point_backup = membase_function::sqlite_chkpoint_count(STORAGE_SERVER_1, $array[0]);
		$this->assertEquals($chk_point_backup, "2", "Checkpoint_mismatch in backup");
		$chk_point_backup = membase_function::sqlite_chkpoint_count(STORAGE_SERVER_1, $array[1]);
