...
abstract class Multi_KV_Store_Config_Param_TestCase extends ZStore_TestCase {
	...
	public function test_Conflicting_DBName(){
		...
		membase_setup::edit_multikv_config_file(TEST_HOST_1 ,"\"dbname\" : \"$db_base\/membase\/ep.db\"","\"dbname\" : \"$db_base\/membase\/ep.db-0.sqlite\"", 'replace');
	...
	public function test_MoreThanOne_DBName(){
		...
		$db_base = (CENTOS_VERSION==5)?"/db":"/data_1";
		membase_setup::clear_membase_log_file(TEST_HOST_1);
		membase_setup::edit_multikv_config_file(TEST_HOST_1 , "\"dbname\" : \"$db_base\/membase\/ep.db\"" , "\"dbname\" : [\"$db_base\/membase\/ep.db\",\"$db_base\/membase\/ep2.db\"]" ,'replace');
		remote_function::remote_execution(TEST_HOST_1, "sudo /etc/init.d/memcached restart");
	...
	public function test_1_data_DBName_10_DBShards(){
		...
		$db_base = (CENTOS_VERSION==5)?"/db":"/data_1";
		membase_setup::edit_multikv_config_file(TEST_HOST_1 , "]",",\"db_shards\" : 10", 'append');
		$status = membase_setup::memcached_service(TEST_HOST_1, "restart");
		$this->assertTrue($status,"Failed to start membase");
		//Check stats            
		$stats = stats_functions::get_kvstore_stats(TEST_HOST_1,"kvstore1");
	...
	public function test_2_data_DBNames_10_DBShards(){
		...
		$db_base = (CENTOS_VERSION==5)?"/db":"/data_1";
		membase_setup::edit_multikv_config_file(TEST_HOST_1 , "\"data_dbnames\"\ :\ \[","\"$db_base\/membase\/ep2.db\",", 'append');
		membase_setup::edit_multikv_config_file(TEST_HOST_1 , "]",",\"db_shards\" : 10", 'append');
		$status = membase_setup::memcached_service(TEST_HOST_1, "restart");
		$this->assertTrue($status,"Failed to start membase");
		//Check stats            
	...
	public function test_1_data_DBName_GreaterThan10_DBShards(){
		...
		membase_setup::clear_membase_log_file(TEST_HOST_1);
		membase_setup::edit_multikv_config_file(TEST_HOST_1 , "]",",\"db_shards\" : 15", 'append');
		remote_function::remote_execution(TEST_HOST_1 ,"sudo /etc/init.d/memcached restart");
		// Compare the entries in the log file
		$command_to_be_executed = "cat ".MEMBASE_LOG_FILE;
		$logs = remote_function::remote_execution(TEST_HOST_1, $command_to_be_executed);
	...
	public function test_1_DBName_1_data_DBName_Invalid_ShardPattern(){
		...
		membase_setup::clear_membase_log_file(TEST_HOST_1);
		membase_setup::edit_multikv_config_file(TEST_HOST_1 , "]",",\"db_shards\" : 4" , 'append' );
	...
	public function test_Missing_KV_Store_Parameter(){
		...
		membase_setup::edit_sysconfig_file(TEST_HOST_1 , ";kvstore_config_file=/etc/sysconfig/memcached_multikvstore_config","",'replace');	
		remote_function::remote_execution(TEST_HOST_1 ,"sudo /etc/init.d/memcached restart");
		// check stats
