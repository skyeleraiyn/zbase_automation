...
abstract class Multi_KV_Store_TestCase extends ZStore_TestCase {
	...
	public function test_Multi_Disk_Goes_Down(){
		...
		$this->assertTrue(Utility::Check_keys_are_persisted(),"Failed persisiting keys");
		// Delete one of the databses
		$command_to_be_executed = "sudo rm -rf /data_*/membase/*";
		remote_function::remote_execution(TEST_HOST_1,$command_to_be_executed);
		$this->assertTrue(Data_generation::verify_added_keys(TEST_HOST_1, 4, "testvalue", 1), "verifying keys failed");
		// Set on a few more keys
	...
	public function est_Membase_restart_with_shard_missing(){ // SEG-10891 - missing shard or missing kvstore file is not detected
		// Membase working with 3 disk and 3 kvstore after one of the disks go down.
		membase_setup::reset_membase_servers(array(TEST_HOST_1));
		flushctl_commands::set_flushctl_parameters(TEST_HOST_1, "chk_max_items", 100);
		$this->assertTrue(Data_generation::add_keys(500, 100, 1, "testvalue"), "adding keys failed");		
		...
		$this->assertTrue(membase_setup::memcached_service(TEST_HOST_1, "stop"));
		// Delete one of the databses
		$command_to_be_executed = "sudo rm -rf /data_2/membase/*";
		remote_function::remote_execution(TEST_HOST_1,$command_to_be_executed);
		$this->assertTrue(membase_setup::memcached_service(TEST_HOST_1, "start"));
		exit; // membase is operational with missing data set

	}

	public function est_Membase_restart_with_one_file_in_shard_missing(){ // SEG-10891 - missing shard or missing kvstore file is not detected 
		// Membase working with 3 disk and 3 kvstore after one of the disks go down.
		membase_setup::reset_membase_servers(array(TEST_HOST_1));
		flushctl_commands::set_flushctl_parameters(TEST_HOST_1, "chk_max_items", 100);
		$this->assertTrue(Data_generation::add_keys(500, 100, 1, "testvalue"), "adding keys failed");		
		...
		$this->assertTrue(membase_setup::memcached_service(TEST_HOST_1, "stop"));
		// Delete one of the databses
		$command_to_be_executed = "sudo rm -rf /data_1/membase/ep.db-0.sqlite*";
		remote_function::remote_execution(TEST_HOST_1,$command_to_be_executed);
