...
class Reshard_test_function{
	...
	public function set_keys($source_machine_list, $destination_machine_list){
		...
		if(installation::verify_php_pecl_DI_capable() and installation::verify_zbase_DI_capable($source_machine_list[0])){
			...
			sleep(1);
			general_function::execute_command("sudo sqlite3 /db/zbase/ep.db-1.sqlite 'update kv set v=\"value\" where k like \"testkey_corrupt_keys_1\";'", $source_machine_list[0]);
			general_function::execute_command("sudo sqlite3 /db/zbase/ep.db-3.sqlite 'update kv set cksum=\"0002:ff\" where k like \"testkey_corrupt_keys_3\";'", $source_machine_list[0]);			
			general_function::execute_command("sudo sqlite3 /db/zbase/ep.db-2.sqlite 'update kv set v=\"value\" where k like \"testkey_corrupt_keys_4\";'", $source_machine_list[0]);
			general_function::execute_command("sudo sqlite3 /db/zbase/ep.db-0.sqlite 'update kv set cksum=\"0002:ff\" where k like \"testkey_corrupt_keys_6\";'", $source_machine_list[0]);			
			general_function::execute_command("sudo sqlite3 /db/zbase/ep.db-2.sqlite 'update kv set v=\"value\" where k like \"testkey_corrupt_keys_0\";'", $source_machine_list[0]);			
