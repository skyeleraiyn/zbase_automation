...
getStatus() {
   ...
   # echo -e "	Count of source key - ( destination key + rejected keys ) :$(($source_count - ($dest_count + $rejected_count)))\n"
    ...
    pdsh -R ssh "ps -ef | grep -q '/opt/membase/bin/ep_engine/management/tap-redistribute-keys.p[y]' || hostname" | sed 's/:.*//' > /tmp/reshard_pdsh_out
...
stop_script() {
	...
	export WCOLL=$source_list
	stop_command="running=\$(ps -ef | grep '/opt/membase/bin/ep_engine/management/tap-redistribute-keys.p[y]') && pid=\$(echo \$running | awk '{print \$2}') && sudo kill \$pid"

	pdsh -R ssh $stop_command > /dev/null 2>&1

	[[ -f /tmp/reshard_pdsh_out ]] && rm /tmp/reshard_pdsh_out
	pdsh -R ssh "ps -ef | grep -q '/opt/membase/bin/ep_engine/management/tap-redistribute-keys.p[y]' && hostname"  > /tmp/reshard_pdsh_out 2>/dev/null
	wc=$(wc -l /tmp/reshard_pdsh_out | awk  '{print $1}')
...
commands='sudo rm -rf /tmp/reshard ;sudo mkdir -p /opt/membase/bin/ep_engine/management' 
export WCOLL=$source_list
pdsh -R ssh $commands
